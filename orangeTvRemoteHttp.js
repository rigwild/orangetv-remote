...
const findAliveHostsIP = async () => (await Promise.all(
	...
	.map(async (_, i) => (await isReachable(`http://${ipPrefix}${i + 1}:${tvPort}`, {
		timeout: 500
	}) && `${ipPrefix}${i+1}`))
)).filter(x => x);
...
module.exports = {
	...
	pushButton: (ipTv, buttonId) => {
		...
		request(ipTv, tvPort, '/remoteControl/cmd', 'GET', querystring.stringify(query))
			.then(() => {
				query.mode = "2";
				return request(ipTv, tvPort, '/remoteControl/cmd', 'GET', querystring.stringify(query));
			})
			.catch(err => console.error('Failed activating the button', err))
			.then(() => console.log("Successfully activated the button"))
			.catch(err => console.error('Failed deactivating the button', err));
	}
};